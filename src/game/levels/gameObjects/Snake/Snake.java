 ...
 */
package game.levels.gameObjects.Snake;
...
public class Snake {
    ...
    public boolean end(){
        ...
        for (int i = 5; i < snake.size()-1; i++) {
            if(!Collision.collisionRactangleBlock(snake_head,snake.get(i)).equals("none") )
            {
                for (GameItem snake_part_ : snake) {
                    snake_part_.grayPng();
                }
    ...
    public boolean eat(){
        try{
          for (GameItem mouse : mouses) {
            if(!Collision.collisionRactangleBlock(snake_head,mouse).equals("none")){
